...
Complete guide for developers working on ContextHub.
...
Output: `contexthub-vscode-1.0.0.vsix`
...
# In Cursor
cursor --install-extension contexthub-vscode-1.0.0.vsix --force
```
...
2. **Read in code**:
```typescript
const config = vscode.workspace.getConfiguration('contexthub');
...
vscode.workspace.onDidChangeConfiguration(e => {
    if (e.affectsConfiguration('contexthub.mySetting')) {
        // Handle change
    }
});
```
...
**Solutions**:
1. Use full command name: "ContextHub: ..."
